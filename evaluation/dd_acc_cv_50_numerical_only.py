...
y = df['ct_wins']
X = df.drop(['ct_wins', 't_wins'], axis=1)
...
X.drop(nominal_columns, inplace=True, axis=1)
results = []
results.append(doAccuracyTests(X, y, 'numerical_only'))


df = pd.DataFrame(results, columns=['encoding', 'knn', 'rfc', 'gnb'])
df.to_csv('./acc/dd_cv50_numerical_only.csv')
