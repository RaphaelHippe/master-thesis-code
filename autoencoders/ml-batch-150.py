...
for S in scaling:
    for LOSS_FN in loss_functions:
        for ACTIVATION_FN in activation_functions:
            ...
            df['y'] = y
            df.to_csv('./encoded_datasets/p100/p100_{}_{}_{}_{}.csv'.format(NAME, S, LOSS_FN, ACTIVATION_FN))
            # X_train, X_test, y_train, y_test = train_test_split(
            #     X_encoded, y, test_size=0.33, random_state=42)
            #
            # print S, LOSS_FN, ACTIVATION_FN
            ...
            # print 'RF', score3
            #
            # key = "{}_{}_{}".format(S, LOSS_FN, ACTIVATION_FN)
            # results.append({
            # 'key': key,
            # 'knn': score1,
            # 'svm': score2,
...
# ax.plot([i for i in xrange(60, len(bar_x))], knn_acc2, 'r-')
# # SVM NOT SCALED
...
# ax.plot([i for i in xrange(60, len(bar_x))], svm_acc2, 'b-')
# # RF NOT SCALED
...
# ax.plot([i for i in xrange(60, len(bar_x))], rf_acc2, 'g-')
#
...
# ax.set_title('Motivation example ({}): Comparison of autoencoders for RF accuracy'.format(COLOR_V))
# plt.xticks(rotation=90)
# # plt.tight_layout(pad=.01)
# plt.gcf().subplots_adjust(bottom=0.5)
# plt.show()
# fig.savefig("./rf-results-{}.png".format(COLOR_V))
