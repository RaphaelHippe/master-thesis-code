...
for encoding_dim in encoding_dims:
    for LOSS_FN in loss_functions:
        for ACTIVATION_FN in activation_functions:
            ...
            autoencoder.add(
                Dense(64, input_shape=(input_dim,), activation=ACTIVATION_FN)
            )
            autoencoder.add(
                Dense(32, activation=ACTIVATION_FN)
            )
            autoencoder.add(
                Dense(16, activation=ACTIVATION_FN)
            )
            autoencoder.add(
            ...
            # plt.show()
            fig.savefig("./images/100c_only/good_bad_plates_autoencoded_dimensions_{}_{}_{}_{}.png".format(NAME, LOSS_FN, ACTIVATION_FN, encoding_dim))
            plt.close('all')
